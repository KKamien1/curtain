...
export const Easing = {
  ...
  // 
  get: function get(type, start, end, t, d, s) {
    return start + Easing[type](Math.min(t, d), d, s) * (end - start);
  },
  getRound: function getRound(type, start, end, t, d, s) {
  ...
  easeInQuad: function easeInQuad(t, d) {
    return (t /= d) * t;
  },
  easeOutQuad: function easeOutQuad(t, d) {
    return -(t /= d) * (t - 2);
  },
  ...
  easeInCubic: function easeInCubic(t, d) {
    return (t /= d) * t * t;
  },
  easeOutCubic: function easeOutCubic(t, d) {
  ...
  easeInQuart: function easeInQuart(t, d) {
    return (t /= d) * t * t * t;
  },
  easeOutQuart: function easeOutQuart(t, d) {
    return -((t = t / d - 1) * t * t * t - 1);
  },
  easeInOutQuart: function easeInOutQuart(t, d) {
    if ((t /= d / 2) < 1) return 1 / 2 * t * t * t * t;
    return -1 / 2 * ((t -= 2) * t * t * t - 2);
  },
  easeInQuint: function easeInQuint(t, d) {
    return (t /= d) * t * t * t * t;
  },
  easeOutQuint: function easeOutQuint(t, d) {
    return (t = t / d - 1) * t * t * t * t + 1;
  },
  easeInOutQuint: function easeInOutQuint(t, d) {
    if ((t /= d / 2) < 1) return 1 / 2 * t * t * t * t * t;
    return 1 / 2 * ((t -= 2) * t * t * t * t + 2);
  },
  easeInSine: function easeInSine(t, d) {
    return -Math.cos(t / d * (Math.PI / 2));
  },
  easeOutSine: function easeOutSine(t, d) {
    return Math.sin(t / d * (Math.PI / 2));
  },
  easeInOutSine: function easeInOutSine(t, d) {
    return -1 / 2 * (Math.cos(Math.PI * t / d) - 1);
  },
  easeInExpo: function easeInExpo(t, d) {
    return t == 0 ? 1 : Math.pow(2, 10 * (t / d - 1));
  },
  easeOutExpo: function easeOutExpo(t, d) {
  ...
  easeInCirc: function easeInCirc(t, d) {
    return -(Math.sqrt(1 - (t /= d) * t) - 1);
  },
  easeOutCirc: function easeOutCirc(t, d) {
  ...
  easeInBack: function easeInBack(t, d, s) {
    if (s == undefined) s = 1.70158;
    return (t /= d) * t * ((s + 1) * t - s);
  },
  easeOutBack: function easeOutBack(t, d, s) {
    if (s == undefined) s = 1.70158;
    return (t = t / d - 1) * t * ((s + 1) * t + s) + 1;
  },
  easeInOutBack: function easeInOutBack(t, d, s) {
    if (s == undefined) s = 1.70158;
